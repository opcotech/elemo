...
[![CI](https://github.com/opcotech/elemo/actions/workflows/ci.yml/badge.svg)](https://github.com/opcotech/elemo/actions/workflows/ci.yml)
...
2. Install [Mise](https://mise.jdx.dev/), then bootstrap the toolchain and demo data

   ```shell
   # Mise installs Go, Node, pnpm, and the rest of the pinned tools from mise.toml.
   # Also required on the host: Docker (with the Compose plugin) and openssl.
   # Bootstrap does not start services or modify databases.
   mise trust
   mise bootstrap
...
3. Start the frontend
   ...
   ```shell
   # Compose is already running after init-demo; this starts the web app too
   mise run dev
   ```
...
- For details on building Elemo, see the
  related [Dockerfile](https://github.com/opcotech/elemo/blob/main/deploy/docker/Dockerfile).
