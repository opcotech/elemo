...
func TestNewPermission(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			args: args{
				subject: ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc}, label: UserIDType},
				target:  ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xc, 0xb, 0xa}, label: RoleIDType},
				kind:    PermissionKindCreate,
			},
			want: Permission{
				ID:      ID{inner: xid.NilID(), label: PermissionIDType},
				Subject: ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc}, label: UserIDType},
				Target:  ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xc, 0xb, 0xa}, label: RoleIDType},
				Kind:    PermissionKindCreate,
			},
...
func TestPermission_Validate(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{
				ID:        ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc}, label: PermissionIDType},
				Kind:      PermissionKindCreate,
				Subject:   ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc}, label: UserIDType},
				Target:    ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xc, 0xb, 0xa}, label: RoleIDType},
				CreatedAt: &time.Time{},
		...
		},
		{
			...
			fields: fields{
				ID:        ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc}, label: PermissionIDType},
				Kind:      PermissionKindCreate,
				Subject:   ID{inner: xid.NilID(), label: ""},
				Target:    ID{inner: xid.ID{0x1, 0x2, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xc, 0xb, 0xa}, label: RoleIDType},
				CreatedAt: &time.Time{},
				UpdatedAt: &time.Time{},
			},
