...
source "$(realpath "$(dirname "${BASH_SOURCE[0]:-$0}")/../../scripts/common.sh")"
...
function getElemoVersion() {
    if [ -z "$(docker images | grep elemo-server)" ]; then
        log "building Elemo"
        dockerCompose -f "${DOCKER_DEPLOY_DIR}/docker-compose.yml" build elemo-server 2>&1
    fi

    log "getting Elemo version"
    formatOutput "Elemo version" "json" $(
        dockerCompose -f "${DOCKER_DEPLOY_DIR}/docker-compose.yml" run \
            -T \
            --rm \
